...
import { MENU_HIGHLIGHTS } from '@/lib/constants/restaurant-data';
...
export function MenuHighlights() {
  ...
  return (
    <section id="menu" className="py-20 md:py-32 px-4 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="mb-6 text-4xl font-bold text-slate-900 md:text-5xl lg:text-6xl">Culinary Excellence</h2>
          <p className="text-base md:text-lg text-slate-600 max-w-3xl mx-auto leading-relaxed">
            Explore our signature dishes, each crafted with passion and precision
          </p>
        </motion.div>
        ...
        <div className="grid gap-8 lg:gap-10 md:grid-cols-2">
          {MENU_HIGHLIGHTS.map((item, index) => (
            ...
            >
              ...
              <p className="mb-6 text-slate-600 leading-relaxed text-base">{item.description}</p>
              <div className="text-3xl font-bold text-amber-600">{item.price}</div>
            </motion.div>
          ))}
        </div>
      </div>
